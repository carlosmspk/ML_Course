...
sys.stdout = f
print ("\n\n\n\n" + "="*65 + "\n\tNEW LOG SESSION: " + datetime.now().strftime("%d/%m/%Y %H:%M:%S") + "\n" + "="*65 + "\n\n")
...
result_train = model.fit(x=batch_gen_train,
...
accuracy_train, accuracy_val = result_train["accuracy"][-1], result_train["val_accuracy"][-1]
print (f"Accuracy (train, val, test): {accuracy_train:.4f}, {accuracy_val:.4f}, {accuracy_test:.4f}")
